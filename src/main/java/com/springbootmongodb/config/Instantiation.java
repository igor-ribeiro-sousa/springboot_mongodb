...
public class Instantiation implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		Post post1 = new Post(null, sdf.parse("2023/07/08"), "Partiu viagem!", "Vou viajar para SP. Abraços!", new AuthorDTO(igor));
		Post post2 = new Post(null, sdf.parse("2023/07/08"), "Bom dia!", "Acordei feliz!", new AuthorDTO(igor));
		
		CommentDTO c1 = new CommentDTO("Boa viagem!", sdf.parse("23/07/10"), new AuthorDTO(mirna));
		CommentDTO c2 = new CommentDTO("Aproveite!", sdf.parse("23/07/10"), new AuthorDTO(virna));
		CommentDTO c3 = new CommentDTO("Tenha um ótimo dia!", sdf.parse("23/07/10"), new AuthorDTO(mirna));
